...
public class Sprite {
  ...
  public Cursor toCursor(int hotX, int hotY) throws TundraException {
    try {
      ...
      float scale =
          (float) width / sheet.getSpriteWidth() * (float) sheet.getWidth() / image.getWidth();

      for (int i = x; i < x + width; i++) {
        for (int j = y; j < y + height; j++) {
          int rgb = image.getRGB((int) Math.floor(i / scale), (int) Math.floor(j / scale));
      ...
      throw new TundraException("Failed to open file", e);
    } catch (LWJGLException e) {
      throw new TundraException("Failed to create cursor", e);
    }
  }
